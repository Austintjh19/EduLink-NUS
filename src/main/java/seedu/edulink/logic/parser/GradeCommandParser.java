...
public class GradeCommandParser implements Parser<GradeCommand> {
    ...
    public GradeCommand parse(String args) throws ParseException {
        ...
        Grade grade;
        requireNonNull(args);
        ArgumentMultimap argMultimap = ArgumentTokenizer.tokenize(args, PREFIX_ID, PREFIX_COURSE, PREFIX_SCORE);

        if (!arePrefixesPresent(argMultimap, PREFIX_ID, PREFIX_COURSE, PREFIX_SCORE)
            || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, GradeCommand.MESSAGE_USAGE));
        }

        argMultimap.verifyNoDuplicatePrefixesFor(PREFIX_ID, PREFIX_COURSE, PREFIX_SCORE);
        try {
